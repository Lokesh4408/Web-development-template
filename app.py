...
def video_feed():
    return Response(get_frame(),
                    mimetype='multipart/x-mixed-replace; boundary=frame') #multiple http responses
...
def motor_status():
    ...
    return str(status)#{ 'success': True }
...
def get_color_from_distance():
    # ...
    #color_from_distance = sensor_controller.get_color_from_distance()
    return jsonify(sensor_controller.get_color_from_distance())#str(color_from_distance)
